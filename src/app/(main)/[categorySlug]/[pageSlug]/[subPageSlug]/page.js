...
export async function getPageData(categorySlug, pageSlug, subPageSlug) {
  try {
    ...
    const res = await fetch(
      `${baseUrl}/api/client/pages/${categorySlug}/${pageSlug}/${subPageSlug}`,
      {
        cache: "no-store",
        headers: {
...
export async function generateMetadata({ params }) {
  const { categorySlug, pageSlug, subPageSlug } = await params;

  const page = await getPageData(categorySlug, pageSlug, subPageSlug);
...
export default async function Page({ params }) {
  ...
  const pageData = await getPageData(categorySlug, pageSlug, subPageSlug);
  ...
  const readableName = pageData.Name
    ...
    <>
      ...
      <div className="min-h-screen bg-[#fafafa] font-sans text-[#1a1a1a]">
        ...
        <header className="mx-auto  max-w-4xl px-6 pt-16 pb-12">
          ...
          <nav
            className="mb-6 hidden flex items-center gap-2 text-xs font-medium text-[#737373]"
          ...
          <Link
            href="/"
            className="group hidden mb-8 inline-flex items-center gap-2 text-xs font-medium uppercase tracking-widest text-[#737373] transition-colors hover:text-[#171717]"
          >
            <svg
