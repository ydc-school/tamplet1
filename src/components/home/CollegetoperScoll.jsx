        ...
        <div className="marquee-inner flex w-fit" style={{ animationPlayState: stopScroll ? "paused" : "running", animationDuration: cardData.length * 2500 + "ms" }}>
          <div className="flex">
            {achievements?.map((card, index) => (
              <div key={index} className="w-72 mx-4 h-[20rem] relative group hover:scale-95 transition-all duration-300 flex-shrink-0">
                <img src={`/uploads/${card?.Image}`} alt="card" className="w-full h-full object-contain" />
