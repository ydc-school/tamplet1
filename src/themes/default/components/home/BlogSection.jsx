            ...
            {blogs.map((blog) => (
              ...
              <SwiperSlide key={blog?.Id} className="h-auto">
                <Link key={blog.Id} href={`/blogs/${blog.Id}`}>
                  <div className="bg-deep-maroon text-white p-8 rounded-sm flex flex-col justify-between h-full text-left relative transition-transform hover:-translate-y-2">
                    <p className="text-body-md font-body-md leading-relaxed mb-8 opacity-90">
                      {blog?.Description}
                    </p>
                    <div className="flex justify-between items-end border-t border-white/20 pt-6">
                      <p className="font-label-md text-label-md tracking-widest uppercase">{blog?.Name}</p>
                      <span className="material-symbols-outlined text-heritage-gold">verified</span>
                    </div>
                  </div>
                </Link>
              </SwiperSlide>
